...
export class AuthService {
    ...
    public checkLogin = () => {
        if (this.cookiesService.has(CONSTS.STORAGE_HELPERS.USER)) {
            this.token = (<any>(
                this.cookiesService.getObject(CONSTS.STORAGE_HELPERS.USER)
            )).token;
        } else {
            this.token = null;
        }
    ...
    public login = ({ login, password }: ILoginRequest) => {
        ...
        return this.http
            .post<ILoginResponse>(`${environment.url_api}/v1.0/users/login`, {
                login,
                password,
            })
            .pipe(
    ...
    public logout = () => {
        this.stopRefreshTokenInterval();
        return this.http
            .delete<any>(`${environment.url_api}/v1.0/users/logout`)
            .pipe(
                map((response) => {
                    this.cleanUserData();
                    this.onlyOneTabActiveService.setActiveTab(
                        OnlyOneTabActiveState.LOGOUT
                    );
                    return response;
                }),
                catchError((error) => {
    ...
    public confirmSupplierLoginSms = ({ confirmationCode }) => {
        return this.http.post<any>(`${environment.url_api}/v1.0/sms/confirm`, {
            confirmationCode,
        });
    };
    ...
    public refreshToken = () => {
        return this.http
            .post<any>(`${environment.url_api}/v1.0/users/refresh`, {
                token: this.token,
            })
            .pipe(
                map((response: ILoginResponse) => {
    ...
    public logoutForced = (isFromUnauthorized = true) => {
        ...
        return this.router.navigate([CONSTS.PATHS.LOGOUT], { state });
    };
    ...
    private getJwtPayload = (token: string = null): IJwtPayload => {
        ...
        if (this.isLogged() || token) {
            ...
            try {
                ...
                jwtPayload.needSmsConfirm =
                    role.indexOf(IUserRoles.NEEDS_SMS_CONFIRMATION) !== -1;
            } catch (e) {
                this.token = null;
                this.cookiesService.remove(CONSTS.STORAGE_HELPERS.USER);
            }
    ...
    private getUserType = (roles: string[]): IUserTypes =>
        R.cond([
            [
                (rolesParam) =>
                    R.indexOf(IUserRoles.ROLE_CONTRACT_IMPORTER)(roles) !== -1,
                R.always(IUserTypes.CONTRACT_IMPORTER),
            ],
            [
                (rolesParam) =>
                    R.indexOf(IUserRoles.PARC_SUPPLIER_P4R)(roles) !== -1,
                R.always(IUserTypes.SUPPLIER),
            ],
            [
                (rolesParam) =>
                    R.indexOf(IUserRoles.PARC_CONSUMER_P_4_R)(roles) !== -1,
                R.always(IUserTypes.CONSUMER),
            ],
    ...
    public isCurrentUser = (userType: IUserTypes) =>
        this.currentUserValue && this.currentUserValue.type === userType;

    public homeRedirect = (
        forceRedirectToLastUrl = false,
        logoutRequired: ILogoutRequired = null
    ) => {
        if (forceRedirectToLastUrl) {
            const lastUrl = localStorage.getItem(
                CONSTS.STORAGE_HELPERS.LAST_URL
            );
            if (lastUrl) {
