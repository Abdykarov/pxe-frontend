...
export class FaqComponent extends AbstractFaqComponent {
    ...
    constructor(
        ...
        combineLatest([this.router.events, this.loadConfigs$])
            ...
            .subscribe(([event, _]) => {
                ...
                const isDetail =
                    (
                        (<NavigationEnd>event).urlAfterRedirects.match(/\//g) ||
                        []
                    ).length === this.NUMBER_OF_SLASH_IN_DETAIL_IN_URL;
                if (isDetail) {
    ...
    private getFaqByTagType = (tagType: string) =>
        R.find(R.pipe(R.prop('tag'), R.head, R.propEq('type', tagType)));

    private getActiveTag = (): string =>
        <string>this.route.firstChild.snapshot?.params?.tag;
}
