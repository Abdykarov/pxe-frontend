...
export class CreateUserComponent extends AbstractComponent {
        ...
        this.router.events.pipe(takeUntil(this.destroy$)).subscribe((event) => {
            if (event instanceof NavigationEnd) {
                ...
                if (askForOfferId) {
                    ...
                    const isNewSupplyPoint = R.path(
                        ['history', 'state', 'isNewSupplyPoint'],
                        window
                    );
                    ...
                    this.supplyPointsImport$ = <Observable<ISupplyPoint[]>>(
                        supplyPointsImport$
                    );
                    this.supplyPointsImportMicroTableData$ = <
                        Observable<IMicroTableData[]>
                    >supplyPointsImportMicroTableData$;
                } else {
                ...
                this.configStepper = getConfigStepper(
                    step,
                    false,
                    TypeStepper.CREATE_USER
                );
            }
        });
    }
    ...
    public editSupplyPoint(supplyPointId: string) {
        this.createUserFacade.setActiveSupplyPoint(null);
        this.router
            .navigate([this.ROUTES.ROUTER_ASK_FOR_OFFER_IN_PROGRESS])
            .then((_) => {
                this.router.navigate(
