...
export class ProgressPriceOfPowerComponent {
    ...
    constructor() {
        this.data = R.forEach(
            (val: IInputDataLineGraph) =>
                (val.date = <any>d3.timeParse('%Y-%m-%d')(val.date))
        )(conf);
    }
    ...
    public mouseMove = (newValue: IDataLineGraph) => {
        const currIndex = R.findIndex(
            (val: IDataLineGraph) =>
                val.date.getTime() === newValue.date.getTime()
        )(this.data);
        this.currValue = this.data[currIndex].value;
        this.prevValue =
            currIndex === 0 ? this.currValue : this.data[currIndex - 1].value;
