    ...
    implements OnInit, AfterViewInit, OnDestroy
{
    ...
    public maxDate: Date = moment()
        .add(-CONSTS.VALIDATORS.ADULTHOOD_AGE, 'years')
        .toDate();
    ...
    ngOnInit() {
        ...
        this.depositPaymentTypeId =
            this.codeLists[CODE_LIST.DEPOSIT_PAYMENT_TYPE];
        const filteredValuesOfDefaultType = R.filter(
            (depositType) =>
                depositType.value === CONSTS.DEFAULT_DEPOSIT_PAYMENT_TYPE_ID,
            this.depositPaymentTypeId
        );

        if (filteredValuesOfDefaultType.length) {
            this.form.controls['depositPaymentTypeId'].setValue(
                CONSTS.DEFAULT_DEPOSIT_PAYMENT_TYPE_ID
            );
        }
        ...
        if (R.path(['contract', 'offer', 'totalPrice'], this.supplyPoint) > 0) {
            this.form.controls['deposit'].setValidators([
                Validators.required,
        ...
        if (this.formValues) {
            ...
            if (this.withPrefillFromProfile) {
                ...
                let personalInfoUnfinished =
                    this.personalInfoLocalStorageService.getPersonalInfo(
                        this.supplyPoint.id
                    );
                if (
                    personalInfoUnfinished &&
                    !R.isEmpty(personalInfoUnfinished)
                ) {
                    if (personalInfoUnfinished.birthDate) {
                        personalInfoUnfinished.birthDate = new Date(
                            personalInfoUnfinished.birthDate
                        );
                    }
                    personalInfoUnfinished =
                ...
                this.form.valueChanges
                    ...
                    .subscribe((_) => {
                        this.personalInfoLocalStorageService.addPersonalInfo(
                            this.supplyPoint.id,
    ...
    ngAfterViewInit() {
        if (
            R.path(['contract', 'offer', 'totalPrice'], this.supplyPoint) > 0 &&
            this.formValues
        ) {
    ...
    public prefillFormData = () => {
        ...
        if (this.formValues) {
            ...
            phone =
                this.formValues.phone && this.formValues.phone.substr(4, 10);
            email = this.formValues.email;
            depositPaymentTypeId =
                this.formValues.depositPaymentType &&
                this.formValues.depositPaymentType?.code;
            deposit = Math.ceil(this.formValues.deposit);
            address1 =
                this.formValues.address1 &&
                R.omit(['__typename'], this.formValues.address1);
            address2 =
                this.formValues.address2 &&
                R.omit(['__typename'], this.formValues.address2);
        }
    ...
    public submitValidForm = () => {
        const form: IPersonalDataInputForm = {
            ...
            phone: this.form.value?.phone
                ? R.concat(CONSTS.TELEPHONE_PREFIX_CZ, this.form.value.phone)
                : null,
            deposit: this.form.value?.deposit
                ? parseFloat(String(this.form.value.deposit).replace(',', '.'))
                : null,
        };
        if (form.birthDate) {
            form.birthDate = convertDateToSendFormatFnc(
                this.form.value?.birthDate
            );
        }
        delete form.phonePrefix;
        delete form[
            'address1' + AddressWhispererComponent.UNIQUE_FIELD_NAME_END
        ];
        delete form[
            'address2' + AddressWhispererComponent.UNIQUE_FIELD_NAME_END
        ];
        delete form.phonePrefix;
