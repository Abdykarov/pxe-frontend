...
export class SupplyPointFormComponent
    ...
    implements OnInit, OnDestroy, OnChanges
{
    ...
    constructor(
        ...
        super(fb);
        const firstDateWitchContractAreNotInProcessTime =
            this.CONSTS.TIME_TO_CONTRACT_END_PROLONGED_IN_DAYS + 1;
        this.minDate = moment()
            .add(firstDateWitchContractAreNotInProcessTime, 'day')
            .toDate();
    }
    ...
    ngOnInit() {
        ...
        this.form
            ...
            .valueChanges.pipe(
                ...
                pairwise(),
                filter(
                    ([prevAnnualConsumptionNTUnit, annualConsumptionNTUnit]) =>
                        prevAnnualConsumptionNTUnit !== annualConsumptionNTUnit
                )
            )
        ...
        this.form
            ...
            .subscribe((commodityType: CommodityType) => {
                ...
                const distributionRateId =
                    commodityType === CommodityType.POWER
                        ? this.getFieldValue('distributionRateId')
                        : null;
        ...
        this.setFormByCommodity(
            this.formValues && this.formValues.commodityType
        );
        this.setContractEndFields();
        this.loadCodeLists();

        combineLatest([this.suppliers$, this.codeLists$])
                        ...
                        this.form.valueChanges
                            .pipe(
                                ...
                                filter(
                                    (_) =>
                                        !this.existsPartialSupplyPointValue &&
                                        R.empty(this.formValues)
                                )
                            )
                            .subscribe((_) => {
                                const formValues = this.form.getRawValue();
                                this.supplyPointLocalStorageService.updateSupplyPoint(
                                    formValues
                                );
                            });
                                ...
                                try {
                                    ...
                                    if (
                                        ...
                                        if (supplyPointForm.expirationDate) {
                                            supplyPointForm.expirationDate =
                                                new Date(
                                                    supplyPointForm.expirationDate
                                                );
                                        }
                                        const setPartialFormForAddress =
                                            AddressWhispererComponent.getAddressNotFoundUniqueValue(
                                                supplyPointForm
                                            );
                                        supplyPointForm =
                                            AddressWhispererComponent.removeAddressNotFoundUnique(
                                                supplyPointForm
                                            );
    ...
    public prefillForm = () => {
        ...
        let contractEndTypeId =
            CONTRACT_END_TYPE.CONTRACT_END_INDEFINITE_PERIOD;
        let timeToContractEnd =
            CONSTS.TIME_TO_CONTRACT_END_INDEFINITE_TIME_IN_MONTHS;
        let timeToContractEndPeriodId: ICodelistItem | string =
            TimeToContractEndPeriod.MONTH;
        ...
        if (!R.isEmpty(this.formValues)) {
            ...
            const supplier = R.find(
                R.propEq('id', this.formValues.supplier?.id)
            )(this.suppliers[commodityType]);
            const expirationDateFromSupplyPoint =
                this.formValues.expirationDate &&
                new Date(this.formValues.expirationDate);
            const expirationDateFromContract =
                this.formValues.contract &&
            ...
            ean =
                this.formValues.commodityType === CommodityType.POWER
                    ? this.formValues.identificationNumber
                    : null;
            eic =
                this.formValues.commodityType === CommodityType.GAS
                    ? this.formValues.identificationNumber
                    : null;
            address =
                this.formValues?.address?.city &&
                R.omit(['__typename'], this.formValues.address);
            distributionRateId =
            ...
            if (
                annualConsumptionVT &&
                annualConsumptionVTUnit === UNIT_OF_PRICES.KWH
            ) {
            ...
            if (
                annualConsumptionNT &&
                annualConsumptionNTUnit === UNIT_OF_PRICES.KWH
            ) {
            ...
            if (
                annualConsumption &&
                annualConsumptionUnit === UNIT_OF_PRICES.KWH
            ) {
        ...
        const filteredContractEndTypeId =
            contractEndTypeId === CONTRACT_END_TYPE.CONTRACT_END_TERMINATE
                ? null
                : contractEndTypeId;
        ...
        this.form.controls['contractEndTypeId'].setValue(
            filteredContractEndTypeId
        );
        this.form.controls['timeToContractEnd'].setValue(timeToContractEnd);
        this.form.controls['timeToContractEndPeriodId'].setValue(
            timeToContractEndPeriodId
        );
    ...
    public submitValidForm = () => {
        const form = {
            ...
            supplierId:
                this.form.value.supplierId && this.form.value.supplierId.id,
            expirationDate:
                this.form.value.expirationDate &&
                convertDateToSendFormatFnc(this.form.value.expirationDate),
        };
    ...
    public removeTerminateFromContractEndType = (codeLists) => {
        const sortEndOfContract = (a, b) => {
            return (
                CONTRACT_END_TYPE_ORDER.indexOf(a.code) -
                CONTRACT_END_TYPE_ORDER.indexOf(b.code)
            );
        };
    ...
    public loadSuppliers = (commodityType) => {
        this.supplyService
            .getSuppliers(commodityType)
            .pipe(takeUntil(this.destroy$))
            .subscribe(({ data }) => {
                this.suppliers[commodityType] = transformSuppliers(
