...
export const importErrorCodes = {
    ...
    [CONSTS.IMPORT_ERROR_CODES.MAX_NUMBER_OF_FILES]:
        'Vložit můžeze pouze jeden soubor.',
    [CONSTS.IMPORT_ERROR_CODES.NO_OFFERS_IN_IMPORT]:
        'Pro zvolenou komoditu nebyl nalezen žádný záznam.',
};

export const askForOfferCodes = {
    [CONSTS.ASK_FOR_OFFER.ERROR_CODES.FILE_TYPE]:
        'Soubory s nepodporovanými formáty nebyly nahrány.',
    [CONSTS.ASK_FOR_OFFER.ERROR_CODES
        .FILE_COUNT]: `Maximální počet souborů je ${CONSTS.ASK_FOR_OFFER.MAX_FILE_COUNT}.`,
    [CONSTS.ASK_FOR_OFFER.ERROR_CODES
        .FILE_SIZE]: `Maximální velikost souboru je ${bytesToSize(
        CONSTS.ASK_FOR_OFFER.MAX_FILE_SIZE
    )}.`,
};
...
export const importErrorViolations = {
    ...
    benefit1InvalidSize:
        'Benefit 1 - pro URL můžete použít maximálně ' +
        CONSTS.VALIDATORS.MAX_LENGTH.BENEFIT_URL +
        ' znaků',
    benefitUrl1InvalidSize:
        'Benefit 1 - pro název můžete použít maximálně ' +
        CONSTS.VALIDATORS.MAX_LENGTH.BENEFIT_NAME +
        ' znaků',
    benefitUrl1InvalidFormat: 'Benefit 1 vyplňte validní URL adresu',
    benefitWithUrl1Invalid: 'Benefit 1 není validní',
    benefit2InvalidSize:
        'Benefit 2 - pro URL můžete použít maximálně ' +
        CONSTS.VALIDATORS.MAX_LENGTH.BENEFIT_URL +
        ' znaků',
    benefitUrl2InvalidSize:
        'Benefit 2 - pro název můžete použít maximálně ' +
        CONSTS.VALIDATORS.MAX_LENGTH.BENEFIT_NAME +
        ' znaků',
    benefitUrl2InvalidFormat: 'Benefit 2 vyplňte validní URL adresu',
    benefitWithUrl2Invalid: 'Benefit 2 není validní',
    benefit3InvalidSize:
        'Benefit 3 - pro URL můžete použít maximálně ' +
        CONSTS.VALIDATORS.MAX_LENGTH.BENEFIT_URL +
        ' znaků',
    benefitUrl3InvalidSize:
        'Benefit 3 - pro název můžete použít maximálně ' +
        CONSTS.VALIDATORS.MAX_LENGTH.BENEFIT_NAME +
        ' znaků',
    benefitUrl3InvalidFormat: 'Benefit 3 vyplňte validní URL adresu.',
    benefitWithUrl3Invalid: 'Benefit 3 není validní',
    benefit4InvalidSize:
        'Benefit 4 - pro URL můžete použít maximálně ' +
        CONSTS.VALIDATORS.MAX_LENGTH.BENEFIT_URL +
        ' znaků',
    benefitUrl4InvalidSize:
        'Benefit 4 - pro název můžete použít maximálně ' +
        CONSTS.VALIDATORS.MAX_LENGTH.BENEFIT_NAME +
        ' znaků',
    ...
    nameInvalidSize:
        'Název nabídky -  použít můžete maximálně ' +
        CONSTS.VALIDATORS.MAX_LENGTH.OFFER_NAME +
        ' znaků.',
    priceGasInvalidFormat:
        'Cena plyn - použít můžete maximálně ' +
        CONSTS.VALIDATORS.MAX_DIGIT_AFTER_DECIMAL_POINT_DEFAULT +
        ' desetinná místa a ' +
        CONSTS.VALIDATORS.MAX_DIGIT_BEFORE_DECIMAL_POINT_DEFAULT +
        ' celých čísel.',
    priceVTInvalidFormat:
        'Cena za VT - použít můžete maximálně ' +
        CONSTS.VALIDATORS.MAX_DIGIT_AFTER_DECIMAL_POINT_DEFAULT +
        ' desetinná místa a ' +
        CONSTS.VALIDATORS.MAX_DIGIT_BEFORE_DECIMAL_POINT_DEFAULT +
        ' celých čísel.',
    priceNTInvalidFormat:
        'Cena za NT - použít můžete maximálně ' +
        CONSTS.VALIDATORS.MAX_DIGIT_AFTER_DECIMAL_POINT_DEFAULT +
        ' desetinná místa a ' +
        CONSTS.VALIDATORS.MAX_DIGIT_BEFORE_DECIMAL_POINT_DEFAULT +
        ' celých čísel.',
    permanentPaymentPriceFormat:
        'Stálá cena - použít můžete maximálně ' +
        CONSTS.VALIDATORS.MAX_DIGIT_AFTER_DECIMAL_POINT_DEFAULT +
        ' desetinná místa a ' +
        CONSTS.VALIDATORS.MAX_DIGIT_BEFORE_DECIMAL_POINT_DEFAULT +
        ' celých čísel.',
    validFromMustBeBeforeValidTo:
        'Datum začátku platnosti nabídky musí být dřívější než její konec.',
    deliveryFromMustBeBeforeDeliveryTo:
        'Datum začátku dodávkového období musí být dřívější než jeho konec.',
};
...
export const oAuthApiError = {
    ...
    SEC0004:
        'Přihlášení se nezdařilo. Přihlašte se pomocí e-mailu (přihlašovacího jména) a hesla.',
...
export const restAPIErrorCodes = {
    ...
    pdfNotAvailableTryLater:
        'Vytvoření dokumentu je právě v procesu. Zkuste ho zobrazit později.',
    accessDenied: 'K tomuto dokumentu nemáte oprávnění.',
};

export const graphQLMessagesDynamic = {
    duplicateSupplyPoints: (ids: string) => [
        `V aplikaci již evidujeme importované odběrné místa: ${JSON.parse(
            ids
        )}.`,
    ],
};
...
export const graphQLMessages = {
    ...
    cannotDeleteContract:
        'Omlouváme se, ale z neočekávaného důvodu se odstoupení od smlouvy nepodařilo. Zkuste to prosím znovu později.',
    cannotSignContract:
        'Omlouváme se, ale z neočekávaného důvodu se nepodařilo smlouvu podepsat. Zkuste to prosím znovu později.',
    forbiddenUpdateOfSupplyPoint:
        'Omlouváme se, ale toto odběrné místo není možné nyní upravit. Zkuste to prosím znovu později.',
    invalidSupplyPointId:
        'Hledané odběrné místo neexistuje nebo pro přístup k němu nemáte oprávnění.',
    noActiveAccountAvailable: 'Účet nelze smazat, protože neexistuje.',
    obsoleteSupplyPointVersion:
        'Kontrakt lze uzavřít pouze na nejnovější verzi OM.',
    unknownSupplyPoint:
        'Hledané odběrné místo neexistuje nebo pro přístup k němu nemáte oprávnění.',
    duplicateOffer:
        'Nabídka nebyla založena, protože totožná nabídka již existuje.',
...
export const cantDeleteAllMarkedOffers =
    'Nepodařilo se nám smazat veškeré označené nabídky, prosím zkuste to znovu.';

export const offerValidityMessages = {
    contractEndWithTerminate:
        'Na základě uvedeného data platnosti aktuální smlouvy můžete ' +
        'vybírat pouze z nabídek od svého aktuálního dodavatele.',
    contractEndWithoutTerminate:
        'Na základě uvedeného data platnosti aktuální smlouvy můžete ' +
        'vybírat pouze z nabídek od svého aktuálního dodavatele.',
};
...
export const errorFieldMessages = {
    address: {
        ...
        invalidCurrentAddressPerson:
            'Zvolená korespondenční adresa není platná.',
        requiredPermanentAddressCompany: 'Vyplňte sídlo společnosti.',
        invalidPermanentAddressCompany:
            'Zvolená adresa sídla společnosti není platná.',
        requiredCurrentAddressCompany: 'Vyplňte fakturační adresu společnosti.',
        invalidCurrentAddressCompany:
            'Zvolená fakturační adresa společnosti není platná.',
    },
    annualConsumption: {
        required: 'Vyplňte roční spotřebu plynu.',
        maxKWh: `Nejvyšší možná spotřeba je ${
    ...
    bankAccountNumber: {
        ...
        accountNumber:
            'Vyplňte správné číslo účtu včetně případného předčíslí s pomlčkou.',
        accountNumberBoth:
            'Vyplňte správné předčíslí (včetně pomlčky) a číslo účtu.',
    ...
    consent: {
        signUp: {
            required:
                'Nezapomeňte na souhlas s Obchodními podmínkami parc4u a na vzetí na ' +
    ...
    date: {
        ...
        formatInterval:
            'Datum vyplňte jako interval ve formátu dd.mm.rrrr - dd.mm.rrrr.',
    ...
    ean: {
        ...
        ean:
            'Správné identifikační číslo odběrného místa má 18 znaků, obsahuje pouze čísla,' +
    ...
    eic: {
        ...
        eic:
            'Správný identifikační kód odběrného místa má 16 znaků, obsahuje pouze čísla a písmena, ' +
            'vždy začíná 27ZG a na 5.-7. pozici nesmí mít číslo 800.',
        nonUniqueEic:
            'Vyplněný identifikační kód odběrného místa již registrujeme.',
    },
    email: {
        required: 'Vyplňte svůj e-mail.',
        email: 'Použijte pouze písmena bez diakritiky, číslice, tečku (.) a zavináč (@).',
        alreadyRegisteredEmail:
            'Tento e-mail je již v aplikaci zaregistrovaný.',
    },
    expirationDate: {
        requiredGas:
            'Doplňte datum, dokdy je platná vaše aktuální smlouva na odběr plynu.',
        requiredPower:
            'Doplňte datum, dokdy je platná vaše aktuální smlouva na odběr elektřiny.',
        isInTerminateInterval:
            'V takto krátké době nejsme schopni zajistit přepis smlouvy.',
        expirationAtLeast30DaysBeforeNewDelivery:
            'Nejsme schopni v takto kratké době uzavřít navazující smlouvu.',
        isInProlongInterval:
            'Vaši aktuální smlouvu už máte automaticky prodlouženou. Vložte správné datum.',
        notEnoughDaysToProcessContract:
            'Smlouva nemůže být změněna, protože nezbývá dostatek času ke zpracování.',
    },
    fullName: {
    ...
    name: {
        required:
            'Vyplňte libovolný název odběrného místa (např. Byt dcera) v maximální délce 50 znaků.',
    },
    number: {
        decimal:
            'Použít můžete pouze čísla a pro desetinné místo čárku nebo tečku.',
        ...
        totalDigitLengthBeforeDecimalPoint:
            'Použít můžete maximálně {maxLength} čísel před desetinnou čárkou.',
    },
    numericalSeries: {
    ...
    password: {
        ...
        fieldsMustNotMatch:
            'Heslo se shoduje s aktuálně nastaveným heslem. Prosím, zvolte jíné.',
        invalidCurrentPassword:
            'Vyplněné heslo se s vaším současným heslem neshoduje.',
    },
    phone: {
    ...
    signatoryPosition: {
        required:
            'Vyplňte funkci, kterou má ve společnosti podepisující osoba.',
        maxlengthSignatoryPosition: 'Použít můžete maximálně 80 znaků.',
    },
    smsCode: {
        required: 'Vyplňte kód, který vám přišel v SMS.',
        invalidSmsCode1:
            'Vyplněný kód se neshoduje s kódem, který přišel v SMS. Zbývají 2 pokusy.',
        invalidSmsCode2:
            'Vyplněný kód se neshoduje s kódem, který přišel v SMS. Zbývá 1 pokus.',
        smsCodeAttemptsExceededLimit:
            'Byl vyčerpán limit pro zadání kódu. Klikněte na "Poslat znovu" a vyplňte kód, který přijde v SMS.',
    },
    street: {
    ...
    string: {
        maxlength:
            'Použít můžete maximálně {requiredLength|number:1.0-3} znaků.',
    },
    supplierId: {
