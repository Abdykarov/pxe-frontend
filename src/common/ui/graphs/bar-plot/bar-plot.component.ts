...
export class BarPlotComponent extends AbstractGraphComponent implements OnInit {
    ...
    @Input()
    public limitForTransformLabel =
        BAR_PLOT_CONSTS.LIMIT_FOR_ROTATION_LABEL_TEXT;

    @Input('data')
    set setData(data: IDataBarPlot[]) {
        this._data = R.forEachObjIndexed(
            (dataBarPlot: IDataBarPlot, id: number) => (dataBarPlot.id = id)
        )(data);
    }
    ...
    protected initGraph(): void {
        ...
        const y = d3
            .scaleLinear()
            .domain([
                d3.min(this.data, (d: IDataBarPlot) => d.value) -
                    this.reservedValueInYAxis,
                d3.max(this.data, (d: IDataBarPlot) => d.value) +
                    this.reservedValueInYAxis,
            ])
            .range([this.height, 0]);

        const u = svg.selectAll('rect').data(this.data);
        ...
        const widthOfColumn: number = parseInt(
            svg.select('.column').attr('width'),
            10
        );
        const withoutTransformLabel =
            widthOfColumn > this.limitForTransformLabel;
        ...
        u.enter()
            ...
            .attr(
                ...
                (d: IDataBarPlot) =>
                    x(d.label) +
                    (withoutTransformLabel
                        ? BAR_PLOT_CONSTS.ROTATION_LABEL_MARGIN
                        : widthOfColumn / 2)
            )
        ...
        svg.append('g')
            .attr(
                'transform',
                `translate(0,${this.height + BAR_PLOT_CONSTS.AXIS_MARGIN_TOP})`
            )
            .attr('class', 'x axis')
            .attr('writing-mode', () => (!withoutTransformLabel ? 'tb' : ''))
            .call(
                d3
                    .axisBottom(x)
                    .tickSize(0)
                    .tickPadding(BAR_PLOT_CONSTS.AXIS_PADDING)
            )
