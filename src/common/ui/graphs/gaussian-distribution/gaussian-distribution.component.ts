...
export class GaussianDistributionComponent
    ...
    implements OnInit
{
    ...
    protected initGraph(): void {
        ...
        const maxFreq = operateNestedProperty(
            ['frequency'],
            R.reduce(R.max, -Infinity),
            this.data
        );
        const maxVal = operateNestedProperty(
            ['value'],
            R.reduce(R.max, -Infinity),
            this.data
        );
        const minVal = operateNestedProperty(
            ['value'],
            R.reduce(R.min, Infinity),
            this.data
        );
        const firstFreq = operateNestedProperty(
            ['frequency'],
            R.head,
            this.data
        );
        const lastFreq = operateNestedProperty(
            ['frequency'],
            R.last,
            this.data
        );
        ...
        const x = d3
            .scaleLinear()
            .domain([
                d3.min(this.data, (d: IDataGaussianDistribution) => d.value) -
                    this.reservedValueInYAxis,
                d3.max(this.data, (d: IDataGaussianDistribution) => d.value) +
                    this.reservedValueInYAxis,
            ])
            .range([0, this.width]);
        ...
        const y = d3
            ...
            .domain([
                ...
                d3.max(
                    this.data,
                    (d: IDataGaussianDistribution) => d.frequency
                ) + this.reservedValueInYAxis,
            ])
            .range([this.height, 0]);
        ...
        this.svg
            ...
            .attr(
                'points',
                `${x(minVal)},${this.height} ${x(maxVal)},${this.height} ${x(
                    maxVal
                )},${y(lastFreq) - 0.5} ${x(minVal)},${y(firstFreq) - 0.5}`
            );
