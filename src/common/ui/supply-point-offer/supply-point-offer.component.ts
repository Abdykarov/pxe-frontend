    ...
    implements OnInit
{
    ...
    ngOnInit() {
        ...
        if (this.questions) {
            const vatNumber = R.path(['supplier', 'vatNumber'])(
                this.supplyPointOffer
            );
            this.question = {
                ...R.find(R.propEq('vatNumber', vatNumber))(this.questions),
            };
            ...
            if (!R.isEmpty(this.question)) {
                ...
                const indexOfLastWord = textWithoutHTML
                    .substr(CONSTS.MAX_LENGTH_SUPPLIER_DESCRIPTION)
                    .indexOf(' ');
                this.question.shortContent = R.pipe(
                    R.take(
                        indexOfLastWord + CONSTS.MAX_LENGTH_SUPPLIER_DESCRIPTION
                    ),
        ...
        this.dateDiffValidityOfOffer =
            this.dateDiffValidityOfOffer ===
            SupplyPointOfferComponent.ZERO_HOURS_VALIDITY_OF_OFFER
                ? SupplyPointOfferComponent.MIN_HOURS_VALIDITY_OF_OFFER_DISPLAYED
                : this.dateDiffValidityOfOffer;

        if (
            this.dateDiffValidityOfOffer <=
                SupplyPointOfferComponent.MAX_HOURS_VALIDITY_OF_OFFER_DISPLAYED &&
        ...
        if (R_.isString(this.supplyPointOffer.benefits)) {
            try {
                this.supplyPointOffer.benefits = JSON.parse(
                    <string>this.supplyPointOffer.benefits
                );
            } catch (e) {
