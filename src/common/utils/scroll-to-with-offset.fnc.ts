...
export const scrollToWithOffset = (el: Element | string, offsetY: number, applyOnlyIfElementIsInViewport = true) => {
    const element: HTMLElement = R_.isString(el) ? document.querySelector(<string>el) : <HTMLElement>el;
    if (element && (!applyOnlyIfElementIsInViewport || !elementInViewport(element))) {
