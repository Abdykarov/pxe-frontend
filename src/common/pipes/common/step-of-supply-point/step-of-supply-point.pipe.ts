...
export class ProgressStatusPipe implements PipeTransform {
    private static readonly SUPPLY_POINT_STATE_TO_TEXT = {
        ...
        [ProgressStatus.PERSONAL_DATA]: (
            ...
            subjectTypeCode === SubjectType.SUBJECT_TYPE_INDIVIDUAL
                ? '2/3 Vyplnění osobních údajů'
                : '2/3 Vyplnění firemních údajů',
    ...
    transform(
        ...
        subjectType: ICodelistItem = null
    ): string {
        let resultText = '';
        const getProgressStatusText =
            ProgressStatusPipe.SUPPLY_POINT_STATE_TO_TEXT[progressStatus];
        if (getProgressStatusText) {
            resultText = getProgressStatusText(
                subjectType ? <SubjectType>subjectType.code : null
            );
        }
        return resultText;
    }
